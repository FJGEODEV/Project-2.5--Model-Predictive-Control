 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N; t++) {
      ...
      fg[0] += CppAD::pow(vars[v_start + t] - vmax, 2);
    }
    ...
    for (int t = 1; t < N; t++) {
      ...
      // avoid swing, use previous actuations
      if (t > 1) {  
        a = vars[a_start + t - 2];
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Set non-actuators to the max negative and positive values.
  for (int t = 0; t < delta_start; t++) {
    vars_lowerbound[t] = -numeric_limits<float>::max();
    vars_upperbound[t] = numeric_limits<float>::max();
  }

  // delta are set (-30, 30) degrees.
  for (int t = delta_start; t < a_start; t++) {
    vars_lowerbound[t] = -0.5236;
    vars_upperbound[t] =  0.5236;
  }
  ...
  // set up lower and upper bound for different parameters
  ...
  vector<double> runresult;
  runresult.push_back(solution.x[delta_start]);
  runresult.push_back(solution.x[a_start]);
  for (int t = 0; t < N-1; t++) {
	  runresult.push_back(solution.x[x_start + t + 1]);
	  runresult.push_back(solution.x[y_start + t + 1]);
  }
